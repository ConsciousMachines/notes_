...
;; proust-pred-bool: for-all, equality, Booleans.
...
;;      | true                      ; Bool-intro0
;;      | false                     ; Bool-intro1
;;      | (bool-ind expr expr expr expr)     ; Bool induction
...
(define (parse-expr s)
  (match s
    ...
    ['true (True)]
    ['false (False)]
    [`(bool-ind ,e1 ,e2 ,e3 ,e4)
        (Bool-ind (parse-expr e1) (parse-expr e2) (parse-expr e3) (parse-expr e4))]
    ['Bool (Bool)]
...
(define (subst oldx newx expr)
  (match expr
    ...
    [(False) (False)]
    [(Bool-ind e1 e2 e3 e4)
       (Bool-ind (subst oldx newx e1) (subst oldx newx e2) (subst oldx newx e3) (subst oldx newx e4))]
    [(Bool) (Bool)]))
...
(define (ae-helper e1 e2 vmap)
  (match (list e1 e2)
    ...
    [(list (Bool-ind e11 e12 e13 e14) (Bool-ind e21 e22 e23 e24))
...
;; strong-reduce: Context Expr -> Expr
...
(define (reduce ctx expr)
  (match expr
    ...
    [(False) (False)]
    [(Bool-ind P tp fp b)
       (define br (reduce ctx b))
       (match br
         [(True) (reduce ctx tp)]
         [(False) (reduce ctx fp)]
         [else (Bool-ind (reduce ctx P) (reduce ctx tp) (reduce ctx fp) br)])]
    [(Bool) (Bool)]))
...
(def 'not '((λ b => (bool-ind (λ b => Bool) true false b))
            :(Bool -> Bool)))

(def 'and '((λ b1 => (λ b2 => (bool-ind (λ b => Bool) b2 false b1)))
            :(Bool -> (Bool -> Bool))))

; ??? why cant i abstract over P
;(def 'not '((λ Q => (λ b => (bool-ind Q false true b)))
;            :(∀ (Q : (Bool -> Type)) -> (∀ (b : Bool) -> (Q b)))))


(def 'P '((λ b => (b = ((and b) b)))
          :(Bool -> Type)))

(def 'ex11 '((λ b => (bool-ind P (eq-refl true) (eq-refl false) b))
             :(∀ (b : Bool) -> (P b))))
