...
// two separate worlds:
// 1. terms/elements of a mathematical domain. bools, ints, strings, lists,
// 2. types/logical-statements in logic world where we apply rules of inference.
//
// relations are the link between worlds: they map elements to logical statements.
// - equality is one such relation. it is specific to the mathematical language.
// - such functions P from terms to types are the properties of the terms.
// - forall is an addon to a relation to say it holds for all elements of a type.
//
// induction is used to prove forall statements for a type.
// - for bools we brute force: apply the formula to true and false.
// - for nats we show base case, and P(n) -> P(s(n)).
//

// note how we never actually compute anything in the traditional sense.
// we can apply the internal function reduce-one to (not true) to see that it
// reduces to false. but otherwise we only have proofs of statements, and
// reduction is used in constructing the proofs.
